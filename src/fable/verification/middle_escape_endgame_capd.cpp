...
struct DirectCorrelatedGraph {
  ...
  // C2 set for the PG2 Taylor enclosure
  //
  //   X(delta) in anchor + tangent*delta + quadratic*delta^2 + defect.
  //
  // Its C0 component contains the complete graph hull.  The first three
  // formal variational columns are the interval vectors T, Q and E; all
  // remaining columns vanish.  Thus the six retained Hessian entries are
  // precisely the directional Taylor coefficients TT,TQ,TE,QQ,QE,EE over
  // the complete C0 domain.  An interval column for E deliberately forgets
  // component correlations and therefore encloses every e in the defect
  // box (at the price of harmless overestimation).
  C2Set pg2_c2_set() const {
    if (!quadratic_mode) {
      throw std::runtime_error("PG2 C2 set requested for C1 graph");
    }
    const int dimension = anchor.dimension();
    const Ival deviation = u_range - u_center;
    const Ival deviation_squared = square_interval(deviation);
    ...
    for (int row = 0; row < dimension; ++row) {
      ...
      for (int column = 0; column < dimension; ++column) {
        c[row][column] = Ival(row == column ? 1 : 0);
        b[row][column] = Ival(row == column ? 1 : 0);
        initial_derivative[row][column] = Ival(0);
      }
      c[row][0] = tangent[row];
...
void set_pg2_c2_mask(C2Solver& solver, int dimension) {
  using Multiindex = capd::vectalg::Multiindex;
  constexpr int directions = 3;
  std::vector<Multiindex> mask;
  // CAPD's C2 Lohner set stores a square C1 matrix even when most formal
  // columns are identically zero.  Retain every first-order multiindex so
  // its internal matrix algebra remains dimension 12; only the expensive
  // second-order mask is directional.
  mask.reserve(dimension + directions * (directions + 1) / 2);
  for (int column = 0; column < dimension; ++column) {
    Multiindex first(dimension);
    first[column] = 1;
    mask.push_back(first);
  }
  for (int left = 0; left < directions; ++left) {
    for (int right = left; right < directions; ++right) {
      Multiindex second(dimension);
      ++second[left];
      ++second[right];
...
// Second-order parameter graph using only the six directional Hessian forms
// generated by T,Q,E.  CAPD stores diagonal Hessian entries as normalized
// Taylor coefficients and mixed entries as mixed derivatives, so the entries
// below are exactly
//
//   qTT=D2P[T,T]/2, cTQ=D2P[T,Q], cTE=D2P[T,E],
//   qQQ=D2P[Q,Q]/2, cQE=D2P[Q,E], qEE=D2P[E,E]/2.
//
// Taylor's theorem on the complete convex C0 hull then gives the PG2 update
// without propagating the other 72 ambient Hessian pairs.
DirectCorrelatedGraph project_graph_pg2(
    const DirectCorrelatedGraph& input, int section_coordinate,
    const Ival& section_value,
    capd::poincare::CrossingDirection direction, int order,
    double tolerance, bool pair23_chart, LegMode mode,
    const char* leg_label) {
  if (!input.quadratic_mode) {
    throw std::runtime_error("PG2 projection received a C1 graph");
  }
  Map field = pair23_chart ? make_pair23_lc_field()
                           : make_direct_lc_field();
  CoordinateSection section(12, section_coordinate, section_value);
  C2Solver solver(field, order);
  ...
  if (section_coordinate == 0) {
    ...
    const bool strict_exchange =
        ...
        !(section_value.rightBound() < strict_lower.leftBound()) &&
        !(section_value.leftBound() > strict_upper.rightBound());
    solver.setMaxStep(strict_exchange ? Ival(1) / Ival(2000)
                                      : Ival(1) / Ival(500));
  }
  set_pg2_c2_mask(solver, 12);
  C2PoincareMap section_map(solver, section, direction);
  section_map.setMaxReturnTime(50.0);
  C2Set set = input.pg2_c2_set();
  Matrix flow_derivative(12, 12), section_derivative(12, 12);
  Hessian flow_quadratic(12, 12), section_quadratic(12, 12);
  Ival interval_return;
  const Vector direct_image = section_map(
      set, flow_derivative, flow_quadratic, interval_return);
  section_map.computeDP(direct_image, flow_derivative, flow_quadratic,
                        section_derivative, section_quadratic,
                        interval_return);

  audit_graph_leg(input, interval_return, order, tolerance, pair23_chart,
                  mode, leg_label);

  // The point-anchor derivative supplies the linear Taylor term.  The C2
  // pass above is evaluated on the complete hull and supplies only the six
  // Lagrange-remainder forms.
  Solver anchor_solver(field, order);
  anchor_solver.setAbsoluteTolerance(tolerance);
  anchor_solver.setRelativeTolerance(tolerance);
  if (section_coordinate == 0) {
  ...
  const Vector anchor_image =
      anchor_map(anchor_set, anchor_flow_derivative, anchor_return);
  const Matrix anchor_section_derivative = anchor_map.computeDP(
      anchor_image, anchor_flow_derivative, anchor_return);
  if (!direct_image[section_coordinate].contains(section_value) ||
      !anchor_image[section_coordinate].contains(section_value)) {
    throw std::runtime_error(std::string(leg_label) +
                             ": missed PG2 Poincare section");
  }

  constexpr int t_direction = 0;
  constexpr int q_direction = 1;
  constexpr int e_direction = 2;
  const int dimension = input.anchor.dimension();
  const Ival deviation = input.u_range - input.u_center;
  const Ival deviation_squared = square_interval(deviation);
  const Ival deviation_cubed = deviation * deviation_squared;
  const Ival deviation_fourth = square_interval(deviation_squared);
  const Vector linear_tangent =
      anchor_section_derivative * input.tangent;
  const Vector linear_quadratic =
  ...
  for (int row = 0; row < dimension; ++row) {
    const Ival q_tt =
        section_quadratic(row, t_direction, t_direction);
    const Ival c_tq =
        section_quadratic(row, t_direction, q_direction);
    const Ival c_te =
        section_quadratic(row, t_direction, e_direction);
    const Ival q_qq =
        section_quadratic(row, q_direction, q_direction);
    const Ival c_qe =
        section_quadratic(row, q_direction, e_direction);
    const Ival q_ee =
        section_quadratic(row, e_direction, e_direction);
    raw_tangent[row] = linear_tangent[row] + c_te;
  ...
  Vector output_quadratic = midpoint_coefficients(
      raw_quadratic, deviation_squared, quadratic_spill);
  Vector output_defect(dimension);
  for (int row = 0; row < dimension; ++row) {
    output_defect[row] = (anchor_image[row] - output_anchor[row]) +
                         raw_defect[row] + tangent_spill[row] +
                         quadratic_spill[row];
    const Ival direct_defect =
    ...
    if (!capd::intervals::intersection(output_defect[row], direct_defect,
                                        sharpened)) {
      throw std::runtime_error(std::string(leg_label) +
                               ": empty PG2 graph-defect intersection");
    }
    output_defect[row] =
        capd::intervals::intervalHull(sharpened, Ival(0));
  }
...
DirectCorrelatedGraph project_graph(
    ...
    double tolerance, bool pair23_chart, LegMode mode,
    const char* leg_label) {
  const bool pg2 = std::getenv("FABLE_ENDGAME_GRAPH_PG2") != nullptr;
  return input.quadratic_mode
             ? (pg2 ? project_graph_pg2(input, section_coordinate,
                                        section_value, direction, order,
                                        tolerance, pair23_chart, mode,
                                        leg_label)
                    : project_graph_c2(input, section_coordinate,
                                       section_value, direction, order,
                                       tolerance, pair23_chart, mode,
                                       leg_label))
