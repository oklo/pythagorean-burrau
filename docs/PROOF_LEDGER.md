...
| Every open reference section-phase arc pulls back to infinitely many intervals containing rational Euclid parameters | PROVED ANALYTICALLY | Uses $\Phi_{\rm sec,ref}'\sim-(3\pi/2)B^{-5/2}$ and density of rational $u$; requires true $o(1)$ circular phase matching in the same gauge before it has dynamical force |
| True skinny raw section phase has circular distance $o(1)$ from $\Phi_{\rm sec,ref}$ | CONJECTURAL | Requires uniform LC averaging across $O(B^{-3/2})$ collision-like binary cycles and collision-free propagation to one fixed regularized section |
