...
| At a simple resonance the finite-$B$ brake condition reduces locally to one angular-slip scalar | CONJECTURAL | Exact Schur-complement reduction is proved, but application to the tied family needs a $C^1$ regularized incoming extension and $D_n\ne0$ |
| First transverse coefficient is $C_n=\xi'(T_n)=-(3/4Z_n)\int r^2zp/d^5\,d\theta$ | EXACT SYMBOLIC IDENTITY | Formal first variation with the stated weighted incoming boundary data; proving those data from the exact Pythagorean family requires first-order matching; nonvanishing is open |
