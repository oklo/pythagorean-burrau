...
### Forced-lag equation and the one-switch reduction

The history comparison has a more useful exact form than the contact test.
On the strict ordered positive-area cone define
...
\[
 \lambda={m\delta(y^{-3}-1)\over W}>0,
 \qquad c={kW\mathcal A\over\delta}>0.
\]

Direct substitution of the exact equations for $\eta_s$ and $k_s$ gives

\[
 \boxed{e_s+(\lambda+c)e=cd},\qquad e(0)=0,
\]

and hence the exact Volterra formula
...
\[
 X={\delta x_s\over W},\qquad Y={\delta y_s\over W},\qquad
 D=d_xX+d_yY,
\]

removes all radicals from the forcing sign:
...
before the terminal syzygy.  More decisively, even a pointwise one-way
theorem for the surface $E=0$ is false under the obvious static constraints.
There is an exact rational witness
...
where $\gamma=(\log R)_s\delta/W$.  It has strict ordered obtuse shape,
$k<\eta<h$, $D<0$, $E=0$, contraction, decreasing area and total moment of
inertia, the launch torque signs $(-,+,-)$, and exact Burrau energy after a
positive scale change, yet

\[
 \boxed{W\delta E_s>0}.
\]

This is an ambient state and is not asserted reachable.  It disproves the
static invariant-region lemma, not the one-switch property of actual brake
trajectories.

### Failed buffered contact barrier and exact contraction dynamics

Put $C=S/P=1/Z_J$, which is regular as $S$ crosses zero, and consider the
buffered contact quantity

\[
 b=h-\eta+y(1-ZC).
\]

At $\eta=h$, $b>0$ implies the sharp safe-amplitude condition when $S<0$
and is automatic when $S\ge0$.  Its full rational derivative, including the
homothetic flux $\chi=(\log R)_sW\delta$, is generated by
`ordered_history_buffered_barrier_reduction`.  Pointwise invariance is
**DISPROVED** by the exact rational state

\[
 u={207\over500},\qquad x={229\over320},\qquad y={131\over320},
 \qquad \eta=k+{3\over4}(h-k),\qquad \gamma=-{1\over1000}.
\]

It obeys the same ordered-obtuse, history-strip, torque-sign, contraction,
and exact-energy compatibility tests, has $b=0$, but
$W\delta b_s<0$.  Again, reachability is deliberately not claimed.

For future history estimates the normalized contraction has also been
derived exactly.  With $Z=W^2$ and
$g_{12}=-(m+n)-(1+y^2-x^2)/(2y^3)-(1+x^2-y^2)/(2x^3)$,
...
The exact kinetic energy is $K=Z(A\gamma^2+B\gamma+C)$, with rational
coefficients in $(m,n,x,y,\eta)$.  Minimizing over $\gamma$ reproduces
one half of the independently derived Gram-inverse pair-torque coefficient,
which is an exact cross-check.  Energy alone still does not make the first
contraction threshold invariant; bounded ordinary probes find both boundary
orientations.  Any successful contraction estimate must therefore use its
integrated evolution from the brake.

Finally, the complete static domain now has a four-cube chart over
$\mathbb Q(\sqrt2)$.  Write $u=(\sqrt2-1)v$, set
$t=(\sqrt2-1)a$ and

\[
 x_0={1-t^2\over1+t^2},\qquad y_0={2t\over1+t^2},\qquad
 r_*={1+\rho(x_0+y_0-1)\over x_0+y_0},
\]

then $x=r_*x_0$, $y=r_*y_0$, and
$\eta=k+r(h-k)$ for $(v,a,\rho,r)\in[0,1]^4$.  The faces $\rho=0$ and
$\rho=1$ are respectively the syzygy and right-triangle boundaries; this
chart covers the full tied ordered right/obtuse history strip and is ready
for exact Bernstein or interval certificates.
