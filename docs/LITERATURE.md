...
| [Féjoz (2001)](https://doi.org/10.1006/jdeq.2000.3972), [author PDF](https://www.ceremade.dauphine.fr/~fejoz/Articles/averaging.pdf) | Planar three-body averaging near inner collisions | Levi--Civita action--angle variables and averaged Hamiltonians | Regularized averaged flow extends across inner collisions | Rigorous averaging/conjugacy result | Not a brake theorem | Supports the coordinate framework, but its secular conjugacy is not the uniform incoming-tail theorem needed here; the early $O(B^{-3/2})$ cycle train is instead controlled by direct LC oscillator estimates |
