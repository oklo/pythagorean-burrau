#!/bin/sh
# Build and run the deterministic CAPD C2 coefficient/return-time probe.
set -eu

PINNED_CAPD_COMMIT=731079217a9254ea2948d742df2b170895effe7f

if [ "$#" -lt 2 ]; then
  echo "usage: $0 CAPD_SOURCE_DIR CAPD_MP_BUILD_DIR [PREC]" >&2
  exit 2
fi
...
OUTPUT_BINARY=${TMPDIR:-/tmp}/capd_c2_semantics_probe

# shellcheck disable=SC2086
c++ -O2 -DFABLE_MP -Wno-deprecated-literal-operator \
  -I"$MPFR_PREFIX/include" \
  "$REPOSITORY_DIR/src/fable/verification/capd_c2_semantics_probe.cpp" \
  $CAPD_FLAGS -L"$MPFR_PREFIX/lib" -lmpfr -lgmp -o "$OUTPUT_BINARY"

"$OUTPUT_BINARY" "$PRECISION"
