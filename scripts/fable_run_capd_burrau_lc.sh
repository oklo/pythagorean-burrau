...
if [ "$#" -lt 2 ]; then
  echo "usage: $0 CAPD_SOURCE_DIR CAPD_MP_BUILD_DIR [PREC TOL ORDER]" >&2
  exit 2
fi
...
OUTPUT_BINARY=${TMPDIR:-/tmp}/fable_burrau_lc_certificate_capd

# shellcheck disable=SC2046
c++ -O2 -DFABLE_MP -Wno-deprecated-literal-operator \
  -I"$MPFR_PREFIX/include" \
  "$REPOSITORY_DIR/src/fable/verification/burrau_lc_certificate_capd.cpp" \
