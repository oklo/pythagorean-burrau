#!/bin/sh
# Build and run the pair-{2,3} interval terminal escape checker.
#
# usage: fable_run_capd_middle_escape.sh CAPD_SOURCE_DIR [CAPD_BUILD_DIR] [--demo]
# The box is read from stdin unless --demo is given; see the header of
# src/fable/verification/middle_escape_terminal_capd.cpp for the format.
set -eu

PINNED_CAPD_COMMIT=731079217a9254ea2948d742df2b170895effe7f

if [ "$#" -lt 1 ]; then
  echo "usage: $0 CAPD_SOURCE_DIR [CAPD_BUILD_DIR] [--demo]" >&2
  exit 2
fi
...
OUTPUT_BINARY=${TMPDIR:-/tmp}/fable_middle_escape_terminal_capd

# shellcheck disable=SC2086
c++ -O2 "$REPOSITORY_DIR/src/fable/verification/middle_escape_terminal_capd.cpp" \
